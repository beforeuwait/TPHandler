...
def add_cookie():
    ...
    svid_cookie = cookie_transform(cookies_list)
    resp = get_request(url=SVID_URL, headers=SVID_HEADERS, params=SVID_PARAMS, cookies=svid_cookie)
    if resp:
...
def check_phone_number():
    ...
    if resp_1:
        ...
        post_request(url=url, headers=headers, data=payloads_2, cookies=cookies)
        hset_name(ID_KEY, 'ex1', p_id)
    return
...
def request_captcha():
    ...
    new_cookie = session_cookie_update(cookies_list, resp.cookies.items())
    save_new_cookie(ID_KEY, new_cookie)
    hset_name(ID_KEY, 'captcha1', captcha)
    return
...
def login_get_html():
    ...
    if resp:
        new_cookie = session_cookie_update(cookies_list, resp.cookies.items())
        cookies_loc1 = cookie_transform(new_cookie)
        location_url = resp.headers.get('Location')
        new_headers = HEADERS_DICT.get('ecs_headers')
        resp_location_1 = get_request(url=location_url, headers=new_headers, cookies=cookies_loc1)
        if resp_location_1:
            new_cookie = session_cookie_update(new_cookie, resp_location_1.cookies.items())
            cookies_loc2 = cookie_transform(new_cookie)
            location_url_2 = resp_location_1.headers.get('Location')
            resp_location_2 = get_request(url=location_url_2, headers=new_headers, cookies=cookies_loc2)
            if resp_location_2:
                new_cookie = session_cookie_update(new_cookie, resp_location_2.cookies.items())
                save_new_cookie(ID_KEY, new_cookie)
                html = resp_location_2.content.decode('utf-8')
                hset_name(ID_KEY, 'html', html)


def do_redirect_get_cloud_session():
    ...
    if resp_1:
        ...
        cookies_2 = cookie_transform(new_cookie)
        resp_2 = get_request(url=url_2, headers=headers, cookies=cookies_2)
        if resp_2:
            new_cookie = session_cookie_update(new_cookie, resp_2.cookies.items())
            url_3 = resp_2.headers.get('Location')
            cookies_3 = cookie_transform(new_cookie)
            resp_3 = get_request(url=url_3, headers=headers, cookies=cookies_3)
            if resp_3:
                new_cookie = session_cookie_update(new_cookie, resp_3.cookies.items())
                url_4 = resp_3.headers.get('Location')
                cookies_4 = cookie_transform(new_cookie)
                headers_prov = HEADERS_DICT.get('each_prov_headers')
                headers_prov.update({'Host': 'sc.189.cn'})
                resp_4 = get_request(url=url_4, headers=headers_prov, cookies=cookies_4)
                if resp_4:
                    new_cookie = session_cookie_update(new_cookie, resp_4.cookies.items())
                    url_5 = resp_4.headers.get('Location')
                    cookies_5 = cookie_transform(new_cookie)
                    headers_prov = HEADERS_DICT.get('each_prov_headers')
                    resp_5 = get_request(url=url_5, header=headers_prov, cookies=cookies_5)
                    if resp_5:
                        ...
                        cookies_home = cookie_transform(new_cookie)
                        resp_home = get_request(url=url_user, headers=headers_home, cookies=cookies_home)
                        if resp_home:
                            new_cookie = session_cookie_update(new_cookie, resp_home.cookies.items())
                            save_new_cookie(ID_KEY, new_cookie)
                            ...
                            hset_name(ID_KEY, 'ex2', ''.join(['http://www.189.cn', fare_link]))
                            hset_name(ID_KEY, 'ex3', ''.join(['http://www.189.cn', detail_link]))
...
def ct_run(tkey):
    ...
    login_url = request_log_index()
    request_login_url(login_url)
    add_cookie()
    check_phone_number()
    request_captcha()
    mail_verify()
